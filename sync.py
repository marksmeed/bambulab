...
We CANNOT call the server directly: it enforces mutual TLS and the client
certificate is non-exportable (see the technical notes, section 1).  Only the
client's own requests (through its cuckoo proxy) carry the cert, so we observe
those requests and read their response bodies — we never originate HTTP
ourselves.  This is the same method bambu_filament.py uses.
...
def _ensure_deps():
    ...
    deps = {"websocket-client": "websocket"}
...
# /file/f3mffolder is only requested when the Files section of the client is
# open.  We wait up to this long; if nothing is seen, the models sync is
# simply skipped this round (the library changes rarely, so this is fine).
...
def sync_models_once() -> bool:
    ...
    if not folders_data:
        print(f"[{_ts()}] sync_models: no /file/f3mffolder seen this round "
              f"(open the client's Files section to refresh the model library) — skipping.")
        return False
